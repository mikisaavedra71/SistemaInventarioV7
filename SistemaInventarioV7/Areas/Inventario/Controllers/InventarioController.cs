    ...
    public class InventarioController : Controller
    {
        ...
        public async Task <IActionResult> GenerarStock(int id)
        {
            ...
            foreach (var item in detalleLista)
            {
                ...
                {
                    await _unidadTrabajo.KardexInventario.RegsitrarKardex(bodegaProducto.Id, "Entrada", "Registro de Inventario",
                                                                          bodegaProducto.Cantidad, item.Cantidad, claim.Value);
                    bodegaProducto.Cantidad += item.Cantidad;
                    await _unidadTrabajo.Guardar();

                }
                else  //Regsito de Stock no existe,hay que crearlo
                {
                    ...
                    await _unidadTrabajo.Guardar();
                    await _unidadTrabajo.KardexInventario.RegsitrarKardex(bodegaProducto.Id, "Entrada", "Inventario Incial",
